...
public class UnityTFFunction
{
    ...
    private List<UnityTFTensor> inputs;
    private List<UnityTFTensor> outputs;
    private string name;
    private List<TFOperation> updates_op;

    public UnityTFFunction(UnityTFBackend k, List<UnityTFTensor> inputs, List<UnityTFTensor> outputs, List<List<UnityTFTensor>> updates, string name)
    {
        this.backend = k;
        this.graph = k.Graph;

        if (updates == null)
            updates = new List<List<UnityTFTensor>>();
        this.inputs = inputs;
        this.outputs = outputs;
        {
            var updates_ops = new List<TFOperation>();
            foreach (List<UnityTFTensor> update in updates)
            {
                if (update.Count == 2)
                {
    ...
    private void PrintVariables(Dictionary<UnityTFTensor, Array> feed_dict, TFSession session)
    {
        string[] ops =
        {
